...
router.post("/", requireAuth, requirePlan, async (req: Request, res: Response) => {
  try {
    ...
    await Session.update(sessionId, { status: "preparing" });
    ...
    runPrepare(
      ...
      async (totalTokens, batchCount) => {
        // Read the distribution-plan.json that the script wrote and sync amounts to PostgreSQL
        ...
        if (fs.existsSync(planPath)) {
          ...
          await Wallet.upsertPlan(sessionId, plan);
        }
